...
public class ShowRoom extends Room {
	 ...
	 * 
	 * @param drink
	 *            specifies for which drink the operation is done
	 * @param quantity
	 *            amount, which shall be bought (and therefore is subtracted)
	 * @return returns 1 if successful and 2 if there is not enough there to
	 *         subtract
	 */
	public int sellDrink(int drink, int quantity) {
		...
		// code never ends here, but for the compiler its needed
		return 0;
	}

	/**
	 * calculates where the amount and max do not match in the show room then
	 * transfers the missing amounts from the chosen storage room to the show room
	 * 
	 * @param roomnumber
	 *            the room from where the show room will be filled
	 * @param storer1
	 *            storage room 1, to apply the possible changes
	 * @param storer2
	 *            storage room 2, to apply the possible changes
	 */
